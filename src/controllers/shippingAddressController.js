...
const checkOwnerShip = (requestedUserId, loggedUser, nextFunction) => {
  if (requestedUserId !== loggedUser.id && loggedUser.role !== 'admin')
    return nextFunction(new AppError('You are not allowed to perfom this action', httpStatus.FORBIDDEN));
};
...
exports.getShippingAddress = catchAsync(async (req, res, next) => {
  ...
  checkOwnerShip(userId, req.user, next);
...
exports.createShippingAddress = catchAsync(async (req, res, next) => {
  ...
  if (filteredData.userId) checkOwnerShip(filteredData.userId, req.user, next);
